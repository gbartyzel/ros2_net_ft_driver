...
bool AtiAxiaFTInterface::set_sampling_rate(int rate)
{
  ...
  const auto response = get_config("netftapi2.xml");
  const int current_rate = std::stoi(parse_config(response, "netft", "setrate"));
  rate = std::clamp(rate, min_sampling_freq_, current_rate);
